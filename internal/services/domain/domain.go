...
func (s *Service) UpdateDomainStatus(domainID int64, isActive bool) error {
	// 检查域名是否存在
	domain, err := s.svcCtx.DomainModel.GetById(domainID)
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
	...
	// 更新域名状态
	err = s.svcCtx.DomainModel.UpdateStatus(nil, domainID, isActive)
	if err != nil {
		return fmt.Errorf("更新域名状态失败: %v", err)
	}

	// 如果禁用域名，需要同时禁用该域名下的所有邮箱
	if !isActive {
		err = s.disableAllEmailsInDomain(domain.Name)
		if err != nil {
			// 记录错误但不回滚域名状态更新
			fmt.Printf("禁用域名 %s 下的邮箱时出错: %v\n", domain.Name, err)
		}
	}
...
// disableAllEmailsInDomain 禁用指定域名下的所有邮箱
func (s *Service) disableAllEmailsInDomain(domainName string) error {
	// 获取该域名下的所有用户
	users, err := s.svcCtx.UserModel.GetUsersByDomain(domainName)
	if err != nil {
		return fmt.Errorf("获取域名下的用户失败: %v", err)
	}

	// 批量禁用用户
	for _, user := range users {
		err = s.svcCtx.UserModel.UpdateStatus(nil, user.Id, false)
		if err != nil {
			fmt.Printf("禁用用户 %s 失败: %v\n", user.Email, err)
		}
	}
