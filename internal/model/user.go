...
// GetUsersByDomain 根据域名获取用户列表
func (m *UserModel) GetUsersByDomain(domainName string) ([]*User, error) {
	var users []*User

	// 通过邮箱地址的域名部分查询用户
	err := m.db.Where("email LIKE ?", "%@"+domainName).Find(&users).Error
	if err != nil {
		return nil, err
	}
...
func (m *UserModel) UpdatePassword(tx *gorm.DB, userID int64, password string) error {
	db := m.db
	if tx != nil {
		db = tx
	}
...
// UpdateEmail 更新用户邮箱
func (m *UserModel) UpdateEmail(tx *gorm.DB, userID int64, email string) error {
	db := m.db
	if tx != nil {
		db = tx
	}

	return db.Model(&User{}).Where("id = ?", userID).Update("email", email).Error
}
